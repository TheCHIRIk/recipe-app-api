...
class PrivateTagsAPiTests(TestCase):
    """TEst teh authorized user tags API"""
    ...
    def test_retrieve_tags(self):
        ...
        m.Tag.objects.create(user=self.user, name='Vegan')
        m.Tag.objects.create(user=self.user, name='Dessert')

        res = self.client.get(TAGS_URL)

        tags = m.Tag.objects.all().order_by('-name')
    ...
    def test_tags_limited_to_user(self):
        ...
        m.Tag.objects.create(user=user2, name='Fruity')
        tag = m.Tag.objects.create(user=self.user, name='Comfort Food')
